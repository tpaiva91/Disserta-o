...
#endif
#endif
#if HAVE_FENV_H && !defined(__CYGWIN__) /* cygwin does not define FENV_H */
#include <fenv.h>
#endif
...
#define EXTRA ""
  ...
  if ( (key=reg_open_key(L"HKEY_CURRENT_USER/SOFTWARE/YAP/Prolog" EXTRA "", FALSE)) &&
	(RegQueryValueEx(key, name, NULL, &type, data, &len) == ERROR_SUCCESS) ) {
    RegCloseKey(key);
    switch(type) {
    case REG_SZ:
      ptr = malloc(len+2);
  ...
  } else if ( (key=reg_open_key(L"HKEY_LOCAL_MACHINE/SOFTWARE/YAP/Prolog" EXTRA "", FALSE)) &&
	(RegQueryValueEx(key, name, NULL, &type, data, &len) == ERROR_SUCCESS) ) {
    RegCloseKey(key);
    switch(type) {
    case REG_SZ:
