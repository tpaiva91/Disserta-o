...
int
Yap_GrowGlobalArena(UInt size)
{
  UInt old_size = ArenaSz(GlobalArena);
  CELL *pt = ArenaLimit(GlobalArena);
  ...
  size /= sizeof(CELL);
  CreateNewArena(ArenaPt(GlobalArena), size+old_size);
  return TRUE;
}
	...
	*ptf++ = d0;
	...
	if (IN_BETWEEN(dvarsmin, ptd0, dvarsmax)) {
	  *ptf++ = (CELL) ptd0;
	  ...
	  CELL new;
	  CELL *max = ArenaLimit(GlobalArena);
	  CELL *base = ArenaPt(GlobalArena);

	  if (base+2*sizeof(attvar_record)/sizeof(CELL) > max-1024) {
	    goto arena_overflow;		      
	  }
	  bp = to_visit;
...
CopyTermToArena(Term t, Term arena, int share, int copy_att_vars, UInt arity, Term *newarena, UInt min_grow)
{
 ...
 error_handler:
  ...
  XREGS[arity+3] = (CELL)newarena;
  {
    ...
    case -5:
      GlobalArenaOverflows++;
      if (!GrowArena(GlobalArena, old_top, old_size, min_grow, arity+3)) {
	Yap_Error(OUT_OF_STACK_ERROR, TermNil, Yap_ErrorMessage);
	return 0L;
      }
      break;
...
p_nb_setarg(void)
{
  ...
  if (pos < 1 || pos > arity)
    return FALSE;
  to = Deref(ARG3);
  to = CopyTermToArena(ARG3, GlobalArena, FALSE, TRUE, 3, &GlobalArena, garena_overflow_size(ArenaPt(GlobalArena)));
  if (to == 0L)
    return FALSE;
  /* restart this in case there was a stack shift */
  dest = Deref(ARG2);
  if (IsPairTerm(dest)) {
...
p_nb_set_shared_arg(void)
{
  ...
  if (pos < 1 || pos > arity)
    return FALSE;
  to = CopyTermToArena(ARG3, GlobalArena, TRUE, TRUE, 3, &GlobalArena, garena_overflow_size(ArenaPt(GlobalArena)));
  if (to == 0L)
    return FALSE;
  /* in case there was garbage collection or stack shifting */
  dest = Deref(ARG2);
  if (IsPairTerm(dest)) {
    destp = RepPair(dest)-1;
  } else {
